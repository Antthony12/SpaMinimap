...
/// <summary>
/// v1.0.0 - Muestra un pequeño mapa superpuesto del circuito de Spa-Francorchamps
/// (version 2025 de VSR Kevin) con un marcador en vivo (pin.png) que rastrea la
/// posición del jugador. El mapa solo aparece mientras el jugador esté cerca del
/// circuito real, usando puntos de telemetría reales (incrustados más abajo) como
/// línea de referencia. La posición/tamaño del mapa se configuran mediante
/// SpaMinimap.ini.
/// </summary>
public class SpaMinimap : Script
{
    ...
    // Posición y tamaño en pantalla de la superposición del mapa, cargados desde SpaMinimap.ini
    ...
    // Telemetría real de una vuelta grabada (~795 puntos, uno cada ~9m), usada
    ...
    private void LoadIni()
    {
        ...
        if (!File.Exists(path))
        {
            Notification.PostTicker("~y~SpaMinimap.ini no encontrado, se usaran valores por defecto.", false);
            return;
        }
    ...
    private float GetIniFloat(string section, string key, float defaultValue)
    {
        ...
        if (ini.TryGetValue(section, out kv) && kv.TryGetValue(key, out raw))
        {
            ...
            Notification.PostTicker("~r~Valor invalido en SpaMinimap.ini [" + section + "] " + key + "=" + raw + ", usando " + defaultValue.ToString(CultureInfo.InvariantCulture), false);
        }
        return defaultValue;
    }
